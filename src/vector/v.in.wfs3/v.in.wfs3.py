...
#               develop further to v.in.wfs3 by Veronica Köß
# PURPOSE:	    WFS support
...
# % key: aoi
# % type: string
# % description: Comma separated coordinates e.g. 335719,5615930,354798,5636274
# % multiple: yes
# % required: no
# %end
...
# % required: no
# % multiple: no
# % label: version of wfs, e.g.:1.0.0 or 2.0.0
# %end
...
def main():
    ...
    try:
        ...
        grass.fatal(
            _("Server couldn't fulfill the request.\nError code: %s") % e.code
        )
    ...
    if flags["l"]:
        ...
        if os.path.exists("wms_capabilities.xml"):
            grass.fatal(_('File called "wms_capabilities.xml" already exists'))
        # os.move() might fail if the temp file is on another volume,
        # so we copy instead
